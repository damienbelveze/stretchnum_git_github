...
$ git checkout -b main
```
la commande ``checkout`` permet de passer d'une branche à l'autre. Par exemple ``git checkout fleur`` permet de passer sur la branche fleur. L'argument -b crée la branche si elle n'existe pas déjà. 
Chaque fois qu'on crée une branche, on va se situer dans cette nouvelle branche. 
...
La commande inverse consiste à retirer de l'index un fichier. Pour cela on va utiliser la commande ``git rm`` (*rm* pour remove). 
